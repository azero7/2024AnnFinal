...
def translate(texts, tokenizer, model, src_lang, tgt_lang, max_length=512):
    tokenizer.src_lang = src_lang
    encoded = tokenizer(texts, return_tensors='pt', padding=True, truncation=True, max_length=max_length)
    encoded = {k: v.to("cpu") for k, v in encoded.items()}  # Ensure tensors are on CPU for now
...
def back_translate(sentences, tokenizer, model, max_length=512):
    start_time = time.time()
    # Translate to English and then back to Chinese
    en_trans = translate(sentences, tokenizer, model, 'zh_CN', 'en_XX', max_length)
    zh_back = translate(en_trans, tokenizer, model, 'en_XX', 'zh_CN', max_length)
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Time taken to back-translate batch: {elapsed_time:.2f} seconds")
    return zh_back

def back_translation(data, tokenizer, model, batch_size=10, max_workers=10):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i in range(0, len(sentences), batch_size):
            ...
            futures.append(executor.submit(back_translate, batch, tokenizer, model))
...
def main():
    ...
    tokenizer, model = load_mbart_models()
    augmented_data = back_translation(data, tokenizer, model, batch_size=10, max_workers=10)
